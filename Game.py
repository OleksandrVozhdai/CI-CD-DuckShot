...
class Game:
    pygame.mixer.init()
    def __init__(self, settings, fullscreen=False, cap=None, screen=None, last_frame=None, bird_speed=None, birdLevelCount=None,
        ...
        # Set initial volume for sound effects based on settings
        self.shot_sound.set_volume(self.settings.get_volume())
        self.reload_sound.set_volume(self.settings.get_volume())
        self.awp_shot_sound.set_volume(self.settings.get_volume())
        ...
        # Ініціалізація last_frame
        self.last_frame = last_frame  # Зберігаємо параметр як атрибут

        # bird sprite-sheet settings
        self.SpritePerRow = 5
        self.Rows = 4
        ...
        # Player stats
        self.level_timer = 40  # If you want to change time for level, you need to make sure that background video length is >= level_timer
        self.score = 0
        self.blink_time = 0
    ...
    def spawn_bird(self):
        ...
        if self.birdLevelCount > 0:
            ...
            if self.levelType < 2:
                ...
                spawn_weight = [50, 30, 10, 9, 1]  # if something goes wrong
